"""Prime RL / Verifiers finance search environment.

This module migrates the legacy SkyRL environment to a Verifiers-native
multi-turn environment that Prime RL can consume.
"""
...
def _parse_search_action(action: str) -> ParsedSearch:
    ...
    if tool_name == "CompanyNameToTickerTool":
        return ParsedSearch(action=action, tool_name=tool_name, lookup_arg=raw_args.strip())
...
def _compute_terminal_reward(completion_text: str, info: dict[str, Any]) -> RewardBreakdown:
...
async def reward_correctness(completion: vf.Messages, info: dict[str, Any] | None) -> float:
