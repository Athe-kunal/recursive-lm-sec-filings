...
@dataclass
class SECFilingsGeneratorConfig(GeneratorConfig):
    """Extended generator config with SEC Filings-specific fields."""

    sec_filings_config_path: str = ""
    sec_filings_traj_dir: str = ""
...
def init_and_run(
    ...
    try:
        ...
        path = (
            Path(generator_cfg.miniswe_traj_dir)
            / f"step_{global_step}"
            / training_phase
        )
        ...
        if agent is not None:
            ...
            try:
                result = evaluate_trajectory(
                    instance, result, sweagent_config, data_source
                )
...
class MiniSweAgentGenerator(SkyRLGymGenerator):
    def __init__(
        ...
        super().__init__(
            generator_cfg, skyrl_gym_cfg, inference_engine_client, tokenizer
        )

        self.http_server_inference_engine_client_host = (
            generator_cfg.inference_engine.http_endpoint_host
        )

        self.http_server_inference_engine_client_port = (
            generator_cfg.inference_engine.http_endpoint_port
        )
        ...
        if self.generator_cfg.chat_template.name_or_path is not None:
            raise NotImplementedError(
                "MiniSWEAgentGenerator doesn't support custom chat template"
            )
    ...
    async def minisweagent_agent_loop(
        ...
        sweagent_config = yaml.safe_load(
            get_config_path(self.generator_cfg.miniswe_config_path).read_text()
        )
