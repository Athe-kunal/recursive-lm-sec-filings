...
def _choose_contexts(markdown_text: str, randomizer: random.Random) -> _ContextSelection:
    ...
    logger.info(f"{len(contexts)=} {paragraph_context is not None=} {table_context is not None=}")
...
def _build_sampling_prompt(record: _FilingRecord, context: str) -> str:
    prompt = (
        f"task_instructions={_SYNTHETIC_QA_TASK_INSTRUCTIONS}\n"
        "requirements:\n"
        "1) One concise, specific financial question.\n"
        "2) One concise factual answer from the context only.\n"
        "3) Use exact values/units when present.\n"
        "4) Output JSON only.\n"
...
async def _request_qa_from_mcp_client(
    ...
    prompt = _build_sampling_prompt(record=record, context=context)
    logger.info(f"{record.ticker_or_company_name=} {record.year=} {record.filing_type=}")
...
async def generate_synthetic_qa_dataset(
    ...
    if max_questions_per_filing_type < min_questions_per_filing_type:
        raise ValueError("max_questions_per_filing_type must be >= min_questions_per_filing_type")
    ...
    for record in records:
        ...
        context_selection = _choose_contexts(markdown_text=markdown_text, randomizer=randomizer)
