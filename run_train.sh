...
# Use rlm_sec.trainer.skyrl_main so gym id ``search`` maps to SECSearchEnv
# (vector_store/search + ticker/year/filing_type), not SkyRL's generic /retrieve env.
uv run --isolated --frozen --extra fsdp -m rlm_sec.trainer.skyrl_main \
